...
const AllContact = () => {
  ...
  useEffect(async () => {
    setLoading(true);
    try {
      const res = await fetch(`https://contact-management-system-3muu.onrender.com/api/mycontacts`, {
        method: "GET",
        headers: {
  ...
  const deleteContact = async (id) => {
    if (window.confirm("Are you sure you want to delete this contact ?")) {
      try {
        const res = await fetch(`https://contact-management-system-3muu.onrender.com/api/delete/${id}`, {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
    ...
    <>
          ...
          <>
            {contacts.length == 0 ? (
              ...
              <>
                ...
                <table className="table table-hover" role="button" >
                  <thead>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)}>
        ...
        <p>
          ...
          <p>
            <strong>Mobile Number</strong>: {modalData.phone}
          </p>
        </Modal.Body>
